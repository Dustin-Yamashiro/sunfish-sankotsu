...
- `assets/images` の `jpg`, `jpeg`, `png` を `theme/assets/images` に WebP 出力する。
- SVG は最適化し、WebP/GIF/AVIF/font/video は必要に応じてコピーする。
- `theme/functions.php` を薄く保ち、テーマ処理を `theme/inc` に分割する。
...
初回は依存関係のインストール、ブロック依存のインストール、wp-env 起動、WordPress 初期設定をまとめて実行します。
...
1. `npm ci` でルート依存を入れる。
2. `npm run blocks:install` で `original-blocks` の依存を入れる。
3. `npm run wp:start` で wp-env を起動する。
4. `npm run wp:setup` でテーマ有効化、初期投稿削除、パーマリンク設定などを行う。
...
4. `theme/localhost.json` を作成し、WordPress から Vite dev server の JS/SCSS を読めるようにする。
...
このコマンドは `.local/vite.pid` に記録された、このリポジトリの Vite だけを停止します。他プロジェクトの Vite プロセスは止めません。`theme/localhost.json` と `.local/urls.json` も削除します。
...
1. `npm run clean` で `theme/assets` と `original-blocks/build` を削除する。
2. `npm run build:vite` で JS/SCSS を `theme/assets` に出力する。
3. `npm run build:assets` で画像、SVG、font、video を処理する。
4. `npm run build:blocks` で `original-blocks` を build する。
...
- `theme/assets/js`
- `theme/assets/images`
- `original-blocks/build`

`assets/images/**/*.jpg`, `jpeg`, `png` は `theme/assets/images/**/*.webp` に変換されます。同じディレクトリに `foo.jpg` と `foo.png` がある場合は、どちらも `foo.webp` になって衝突するため build を失敗させます。
...
- `theme/inc`: テーマの PHP 処理を責務別に分ける場所。
- `theme/assets`: build 生成物。手で編集しない。
- `original-blocks`: カスタムブロック用プラグイン。
...
- `theme/functions.php` は `theme/inc/*.php` を読み込むだけにする。
- assets 関連は `theme/inc/assets.php` に書く。
- helper は `theme/inc/helpers.php` に書く。
- カスタム投稿や taxonomy は `theme/inc/post-types.php` に書く。
...
アニメーション方針の正式な管理場所は `docs/animation.md` です。GSAP、ScrollTrigger、Splide は `theme/inc/assets.php` で vendor asset として登録済みです。必要なテンプレートや機能でだけ enqueue し、全ページへ無条件に読み込まないでください。
...
npm audit --prefix original-blocks --audit-level=low
```
