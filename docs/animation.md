...
Splide の標準 CSS は `splide` style handle として登録済みです。デザインに合わせて完全に上書きする場合は、Splide の構造 class を使いながら、共通部品なら `assets/scss/object/component`、セクション内の配置なら `assets/scss/layout` に必要なスタイルを書きます。
...
- 特定セクションだけの演出: `assets/scss/layout/_*.scss`
